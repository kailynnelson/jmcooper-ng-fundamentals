...
export class EventService {
	// to make this asynchronous, we'll use an rxjs observable
	// blah blah something resolve blah

	getEvents() {
		let subject = new Subject(); // SUBJECT is a type of OBSERVABLE
		setTimeout(
			// js's method to make it seem asynchronous (don't really know what this means yet)
			() => {
				subject.next(EVENTS);
				subject.complete();
			},
			2000 // after 'n' ms, we'll add data to the stream
		);
		return subject; // return the observable
		// return EVENTS; // return data directly
	}

	getEvent(id: number) {
		// again, will eventually be an ajax call; for now, just pulling off the array below
